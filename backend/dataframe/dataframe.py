...
class Dataframe():
    ...
    def Trim_All_Columns(self, Columns: list[str]) -> None:
        self.Dataframe[Columns] = self.Dataframe[Columns].apply(lambda row: row.str.strip())

    def Removing_Character(self, Column: str, Remove_Character=" ", New_Character="") -> None:
        self.Dataframe[Column] = self.Dataframe[Column].str.replace(Remove_Character, New_Character)

    def Group_By(self, Grouped_Column: str):
        return self.Dataframe.groupby(Grouped_Column)
